module Foraneus
  ...
  #
  # Concrete classes model a set of params/values that are parsed from an external source
  #   (like HTTP query params).
  class ValueSet

    # Builds an instance of ValueSet that conforms with its fields specification.
    #
    # If any of the params can be parsed by its corresponding converter then an
    # {InvalidValueSet} and {RawValueSet} are returned that holds raw_values.
    #
